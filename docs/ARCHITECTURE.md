...
`percentage` and `percentageLabel` retain the compact cross-platform compatibility field. `usageRows` is the ordered native detail contract: the shared engine owns labels, actual percentages, expected percentages, reset countdowns, and per-row severity. Swift renders expected-first comparison meters that keep common usage neutral and color only the difference; GTK retains separate actual and expected bars. Expected usage is the rounded share of the real quota window that has elapsed; monthly Cursor rows use the returned billing-cycle start and end rather than a calendar approximation. The shared tooltip omits normal pacing prose and appends `warning` or `critical` only to the quota window that triggered that severity.
...
The menu bar displays signed weekly pace for connected Anthropic, OpenAI, and SpaceXAI accounts. The popover renders all three fixed columns, including disconnected providers; provider selection remains a Linux-only interaction. Comparable macOS rows place Expected above Actual, show a compact signed `expected − actual` delta, and color only the non-overlapping meter segment green, orange, or red. Unavailable quota lines are removed from the card body and represented by one gray information control beside the provider's update time; its popover explains which provider response omitted the quota. Missing credentials or an unauthorized response set `authenticationRequired`, remove only that provider from the menu-bar title, and expose **Reconnect** in its column. Temporary network, rate-limit, and organization-policy errors remain visible without being mislabeled as disconnected.
