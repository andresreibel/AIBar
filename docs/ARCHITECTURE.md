...
The dropdown uses the status button's display and fits within its usable screen area with a 12-point margin. It retains a 620×450 content size when space allows, accounts for the popover's window chrome, and constrains its outer position after presentation and display changes. Smaller areas use fewer provider columns and vertical scrolling; the refresh header and notification control remain outside the scrolling area. `AIBarPopoverLayout` owns the screen-coordinate calculations, including displays with negative origins.
